...
p_interactive <- card_individual %>%
  select(meta_card_num, meta_pin, {{ target_feature_shap }}, {{ target_feature_value }}) %>%
  mutate(bin = !!sym(target_feature_value)) %>%
  group_by(bin) %>%
  filter(bin %in% c(
    sort(unique(bin), decreasing = TRUE)[1:5], # Top 5 highest values
    sort(unique(bin))[1:5] # Bottom 5 lowest values
  ))
...
```{r SHAP_scatterplot, echo=FALSE,warning=FALSE,message=FALSE,error=FALSE, results='asis',fig.keep='all'}
...
shapviz::shapviz(
  ...
  scale_x_discrete(labels = function(labels) {
    substr(labels, 1, 15) # Truncate to the first 15 characters
  })
```
:::
