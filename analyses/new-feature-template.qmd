...
if (type == "continuous") {
  ...
  pin_nbhd <- pin_individual %>%
    ...
    group_by(meta_nbhd_code) %>%
    mutate(
      !!paste0(target_feature_value, "_neighborhood_mode") := mode_function(!!sym(target_feature_value))
    ) %>%
    ...
    ungroup() %>%
    select(
      meta_nbhd_code, !!sym(target_feature_value), percentage, plurality_factor,
      !!paste0(target_feature_value, "_neighborhood_mode")
    ) %>%
    pivot_wider(
...
`r if (params$type = "continuous") ## Feature Histogram`
...
`r if (parm$type = "continuous")# Correlations`

`r if (parm$type = "continuous")::: panel-tabset`

`r if (parm$type = "continuous")## Correlation Between Added Feature and Other Features`

`r if (parm$type = "continuous") Here, the goal is to see if the added feature *very* neatly aligns with other existing features. Columns are produced with both the absolute value of the correlation (for easy sorting), as well as the raw value to help decipher the direction of the relationship.`
...
`r if (parm$type = "continuous")## Correlation Plot of 10 Features (absolute value)`
...
`r if (parm$type = "continuous") ### Violin Plot Comparing SHAP to Feature`
...
if (params$type == "continuous") {
  ...
  card_individual %>%
    ...
    mutate(bin = cut_number(!!sym(target_feature_value), n = 2, dig.lab = num_digits)) %>%
...
`r if (params$type == "continuous") "###  cat("### Mean Value of the Feature")`
...
`r if (params$type == "continuous") "###  cat("### Median Value of the Feature")`
...
`r if (params$type == "continuous") "### Lowest 100 Values\n\nBe careful interpreting values which are the max and min of the raw value, since ties are not accounted for. For example, if there are 10,000 parcels which are 0 feet from a newly constructed building, the map will not be a full representation."`
