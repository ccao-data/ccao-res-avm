...
outliers <- datasets_sold %>%
  map(\(x) {
    x %>%
      ...
      filter(
        rank(-ratio) <= 10 | rank(ratio) <= 10 # Filter top 10 and bottom 10 based on ratio
      ) %>%
...
```{r, _challenge_groups_outliers_large_lots}
outliers$"Large Lots" %>% 
  kable(
    format = "html",
    digits = 2,
...
```{r, _challenge_groups_outliers_multicard}
outliers$Multicard %>% 
  kable(
    format = "html",
    digits = 2,
...
```{r _challenge_groups_outliers_multifamily}
outliers$Multifamily %>% 
  kable(
    format = "html",
    digits = 2,
    row.names = FALSE,
...
```{r _challenge_groups_outliers_top_decile}
outliers$"Top Decile" %>% 
  kable(
    format = "html",
    digits = 2,
